...
LawCast는 국회 입법예고 변동을 수집하고, Discord 웹훅 알림과 웹 UI로 제공하는 셀프호스트형 플랫폼입니다.

이 저장소는 백엔드/프론트엔드 서브모듈을 묶는 루트 오케스트레이션 레이어로, 다음 역할에 집중합니다.
...
루트 README는 "빠르게 이해하고 바로 실행"하는 허브 문서입니다.

- 백엔드 내부 아키텍처/크론/데이터 무결성 상세는 백엔드 문서에서 관리
- UI 구조/페이지/필터 UX 상세는 프론트엔드 문서에서 관리
- 루트는 시스템 관점(구성, 실행 순서, 운영 스크립트)에만 집중
